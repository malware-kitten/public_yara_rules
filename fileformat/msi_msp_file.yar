private rule MSI_MSP_File {
  meta:
    description = "rule to ID MSI or MSP files clsid's Borrowed from https://opensource.apple.com/source/file/file-54/file/src/readcdf.c "
  ...
  strings:
    ...
    //clsid a and b for the MSI format should exist at offset $root_entry + 0x50
    $msi_clsid_a = { 84 10 0c 00 00 00 00 00 }
    $msi_clsid_b = { c0 00 00 00 00 00 00 46 }

    //\x86\x10\x0c\x00\x00\x00\x00\x00\xc0\x00\x00\x00\x00\x00\x00\x46
    $msp_clsid_a = { 86 10 0c 00 00 00 00 00}
    $msp_clsid_b = { c0 00 00 00 00 00 00 46}

  condition:
    $header_compound at 0 and
    for all of ($root_entry*) :
       ( ($msi_clsid_a at @+0x50 and
         $msi_clsid_b at @+0x50+8) or
     ($msp_clsid_a at @+0x50 and
          $msp_clsid_b at @+0x50+8)
       )
}
